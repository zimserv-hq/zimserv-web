  ...
  return (
    <>
      <style>{`
        ...
        .providers-table tbody tr:hover { background: var(--hover-bg); }
        .providers-table tbody tr:last-child { border-bottom: none; }
        .providers-table td {
          padding: 14px 16px;
        ...
        .provider-info h4 {
          ...
          margin: 0 0 3px 0; display: flex; align-items: center;
        ...
        .needs-review-badge {
          ...
          background: var(--orange-light-bg); color: var(--orange-primary);
          font-size: 10px; font-weight: 700; white-space: nowrap;
          animation: pulse 2s ease-in-out infinite;
        }
        @keyframes pulse { 0%,100%{opacity:1} 50%{opacity:0.7} }
        .provider-info p { font-size: 13px; color: var(--text-secondary); margin: 0; }
        ...
        .status-badge.active    { background: var(--green-light-bg); color: var(--green-dark); }
        .status-badge.pending   { background: var(--yellow-bg);      color: var(--yellow-text); }
        .status-badge.suspended { background: var(--red-bg);         color: var(--red-text); }
        .status-dot { width: 6px; height: 6px; border-radius: 50%; }
        .status-badge.active    .status-dot { background: var(--green-dark); }
        .status-badge.pending   .status-dot { background: var(--yellow-text); }
        .status-badge.suspended .status-dot { background: var(--red-text); }
        ...
        .action-btn.view    { border-color: var(--blue-primary);  color: var(--blue-primary); }
        .action-btn.view:hover { background: var(--blue-light-bg); }
        .action-btn.approve { border-color: var(--green-primary); color: var(--green-primary); }
        .action-btn.approve:hover { background: var(--green-light-bg); }
        .action-btn.review  {
        ...
        .dark-mode .provider-card:hover { box-shadow: 0 2px 8px rgba(0,0,0,0.3); }
        .card-header { display: flex; gap: 12px; margin-bottom: 12px; }
        .card-avatar {
          width: 56px; height: 56px; border-radius: 10px;
          object-fit: cover; border: 1.5px solid var(--border-color); flex-shrink: 0;
        }
        .card-info { flex: 1; min-width: 0; }
        .card-name {
          font-size: 15px; font-weight: 600; color: var(--text-primary);
          margin: 0 0 4px 0; display: flex; align-items: center;
          gap: 6px; flex-wrap: wrap;
        }
        .card-email {
        ...
        .empty-state {
          ...
          border-radius: 12px; background: var(--empty-bg);
        }
        ...
        .skeleton {
          background: linear-gradient(90deg,
            var(--skeleton-from) 25%, var(--skeleton-to) 50%, var(--skeleton-from) 75%);
        ...
        .skeleton-row { height: 70px; margin-bottom: 1px; }

        @media (max-width: 1024px) and (min-width: 769px) {
          .table-container { display: none; }
          .mobile-cards { display: grid; grid-template-columns: repeat(2,1fr); gap: 16px; }
        }
        @media (max-width: 768px) {
          .table-container { display: none; }
          .mobile-cards { display: grid; grid-template-columns: 1fr; gap: 12px; }
        }
      `}</style>
                  ...
                  <td>
                    <div className="action-buttons">
                      {provider.hasPendingEdits && (
                        ...
                        >
                          <AlertCircle size={14} strokeWidth={2.5} /> Review
                        </button>
                      )}
                      <button
        ...
        {providers.map((provider) => {
          ...
          return (
            <div key={provider.id} className="provider-card">
              ...
              {/* Jobs stat REMOVED — Rating + Reviews only */}
